...
public class GolfBallSim extends AttractorSim implements SimulatedObject,SocRef,NewSim 
	{
	...
	public	int	eastscore = 0;
	public	int	westscore = 0;
        public  String estado = "";
	/*--- shot clock time ---*/
	private	static final double TIMEOUT		= 60.0;
	public	double	shotclock = TIMEOUT;
	...
	public GolfBallSim()
        ...
        public String getStat(boolean type){
            return null;
        }
	...
	public void takeStep(long time_increment, SimulatedObject[] all_objs)
		...
		*/
		...
		if ((position.x > 1.37)&&(Math.abs(position.y)<0.25))
			...
			westscore++;
                        this.estado += "" + westscore + "," + eastscore + "," + (int)shotclock +"\n";
			position.setr(0);
			velocity.setr(0);
			shotclock = TIMEOUT;
			//System.out.println("AQUI1 " + westscore+" "+eastscore);
			}
			...
			velocity.setr(0);
			shotclock = TIMEOUT;
			//System.out.println("AQUI2 " + westscore+" "+eastscore);
			}
	 ...
	 */
		...
		g.drawString("score 2: "+westscore+":"+eastscore+
			" shot: "+(int)shotclock,
			(int)((-1.39 -l)/meterspp),
